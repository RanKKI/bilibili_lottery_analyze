...
from network import get as network_get
...
class Bilibili(object):
    ...
    def __init__(self, post_id, user_id, dynamic_id):
        ...
        self.cache_dir = f"./cache/{self.dynamic_id}/"
    ...
    def find_origin_post(self, space: Space) -> Card:
        ...
        如果没有找到抽奖动态, 转至下一页
        """
        return self.find_origin_post(self.get_user_space(space.user.uid, offset=history.dynamic_id))

    def write_prize_users(self, ids: List[int]):
        with open('./output/prize_users.csv', 'w', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile, delimiter=",")
            writer.writerow(self.header)
            for space in map(lambda uid: self.get_user_space(uid), ids):
